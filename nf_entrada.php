...
function criar_nf_entrada(){
    ...
    $headers = array(
        ...
        $query2 = $ConexaoREPORTS->query("select pedido ");
    ...
    foreach ($rows as $row) {
       ...
       // preciso da chave nf para montar a nota de entrada.
        if(isset($respostachavenf0['ListaNfe'][0]['chave_nfe']) and $respostachavenf0['cancelada'] == 'N'){
            ...
            if(array_key_exists(substr($row['numero_pedido'], -2), $variavel_ar)){
            
                $query2 = $ConexaoREPORTS->query("SELECT * FROM registro_reenvio where numero_pedido = '".$pedido_original.$variavel_ar[substr($row['numero_pedido'], -2)]."'");
                $rows = $query2->fetch();
                if($rows){
                    $codigo_original = $rows['codigo_pedido'];
                    $pedido_original = $rows['numero_pedido'];
                }
            }
            ...
            $data_sts = array(
                ...
                'app_secret' => $arrayCredenciaisS[$empresaAtual],
                'param' => array(
                    array('codigo_pedido' => $codigo_original), 
                ),
            );
            $optionsnf = array(
                CURLOPT_URL => $url,
            ...
            // preciso da chave nf para montar a nota de entrada.
            if(isset($respostachavenf['ListaNfe'][0]['chave_nfe']) and $respostachavenf['cancelada'] == 'N'){ 
                ...
                if(isset($resposta['pedido_venda_produto'])){
                    ...
                    if($row['motivo'] == 'produto' or $row['motivo'] == 'endereco_e_produto' or $row['motivo'] == 'composicao_incorreta'){
                            ...
                            foreach($resposta_novo['pedido_venda_produto']['det'] as $detnv){
                                ...
                                foreach($resposta['pedido_venda_produto']['det'] as $det){
                                    ...
                                    if(isset($arrayCFOP[$detnv['produto']['cfop']]) and $arrayCFOP[$detnv['produto']['cfop']] != '' and $detnv['produto']['codigo'] == $det['produto']['codigo']){
                                        ...
                                        $codigo_local_estoque = $detnv['inf_adic']['codigo_local_estoque'];
                                        $nQtd = $detnv['produto']['quantidade'];
                                        $nValUnit = $det['produto']['valor_total'] / $detnv['produto']['quantidade'];
                                ...
                                if($entrou == 0){
                                    foreach($resposta['pedido_venda_produto']['det'] as $det){
                                        $valor = 0;
                                        if($detnv['produto']['codigo'] == $det['produto']['codigo']){
                                            $valor = $det['produto']['valor_total'];
                                        }else{
                                            $valor = $detnv['produto']['valor_total'];
                                        }
                                    }
                                    $cCFOP = $arrayCFOP[$detnv['produto']['cfop']];
                                    ...
                                    $codigo_local_estoque = $detnv['inf_adic']['codigo_local_estoque'];
                                    $nQtd = $detnv['produto']['quantidade'];
                                    $nValUnit = $valor / $det['produto']['quantidade'];
        
                                    $cSitTribPIS = $detnv['imposto']['pis_padrao']['cod_sit_trib_pis'];
                                    $cSitTribCOFINS = $detnv['imposto']['cofins_padrao']['cod_sit_trib_cofins'];
                                    $cSitTrib = $detnv['imposto']['icms']['cod_sit_trib_icms'];
                                    $cOrigem = $detnv['imposto']['icms']['origem_icms'];
                                    $nAliq = $detnv['imposto']['icms']['aliq_icms'];
                                    $nAliqCOFINS = $detnv['imposto']['cofins_padrao']['aliq_cofins'];
                    ...
                    if(!isset($resposta2['faultstring'])){
                        $msg = "UPDATE registro_reenvio set nf_entrada = '".$resposta2['nCodNotaEnt']."', cod_nf_entrada = '".$resposta2['cCodIntNotaEnt']."' where numero_pedido_original  = '".$row['numero_pedido_original']."' and numero_pedido = '".$novo_pedido."';";
                        file_put_contents("update.txt", PHP_EOL .$msg, FILE_APPEND );
                        if($ConexaoREPORTS->query("UPDATE registro_reenvio set nf_entrada = '".$resposta2['nCodNotaEnt']."', cod_nf_entrada = '".$resposta2['cCodIntNotaEnt']."' where numero_pedido_original  = '".$row['numero_pedido_original']."' and numero_pedido = '".$novo_pedido."'")){
                            echo $row['numero_pedido_original'].  ' - '.$resposta2['nCodNotaEnt'].' - '.$resposta2['cCodIntNotaEnt'].' - deu certo'.PHP_EOL;
                        }
                    }else {
                        echo $row['numero_pedido'].' - '.$pedido_original. ' - '.$resposta2['faultstring'].PHP_EOL;
                    }
                }else{
            ...
            if(!isset($resposta2['faultstring'])){
                ...
                if(strstr(mb_convert_encoding($resposta2['faultstring'], 'Windows-1252', 'UTF-8'), 'Nota de Entrada foi conclu')){
                    ...
                    $options2 = array(
                        ...
                        CURLOPT_RETURNTRANSFER => true,
                        CURLOPT_POST => true,
                        CURLOPT_POSTFIELDS => json_encode($dataconsultar),
                    );
...
function checarNF(){
    ...
    $array_pedidos = array();
    $query2 = $ConexaoREPORTS->query("select do pedido");
    $rows = $query2->fetchAll();
    foreach ($rows as $row) {
       $array_pedidos[] = $row['numero_pedido_original'];
    }

    $empresaAtual = 'blablabla';
    ...
    for ($pagina = 1; $pagina <= $totalpagina; $pagina++) {
        ...
        foreach ($resposta['notas'] as $nf) {
            ...
            if (in_array($pedidonf, $array_pedidos)) {
                $msg = "UPDATE registro_reenvio set nf_entrada = '" . $nf['cabec']['nCodNotaEnt'] . "', cod_nf_entrada = '" . $nf['cabec']['cCodIntNotaEnt'] . "' where numero_pedido_original  = '" . $pedidonf . "' and numero_pedido = '" . $pedidonf . "";
                file_put_contents("update.txt", PHP_EOL . $msg, FILE_APPEND);
            }
        }
    }
